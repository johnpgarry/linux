		...
		struct folio *folio)
{
	...
	pr_err("%s calling iomap_set_range_uptodate\n", __func__);
		...
		size_t len, struct folio *folio)
{
	...
	do {
		pr_err("%s3 calling iomap_adjust_read_range UNSHARE set=%d pos=%lld len=%zd block_size=%lld block_start=%lld block_end=%lld nr_blocks=%d\n",
			__func__, !!(iter->flags & IOMAP_UNSHARE),
			pos, len, block_size, block_start, block_end, nr_blocks);
		iomap_adjust_read_range(iter->inode, folio, &block_start,
				block_end - block_start, &poff, &plen);

		pr_err("%s4 called iomap_adjust_read_range UNSHARE set=%d pos=%lld len=%zd block_size=%lld block_start=%lld block_end=%lld nr_blocks=%d poff=%zd plen=%zd\n",
			__func__, !!(iter->flags & IOMAP_UNSHARE),
			pos, len, block_size, block_start, block_end, nr_blocks,
			poff, plen);
		if (plen == 0)
		...
		pr_err("%s5 calling iomap_set_range_uptodate\n", __func__);
		iomap_set_range_uptodate(folio, poff, plen);
		...
		size_t len, struct folio **foliop)
{
	...
	if (special_print && (is_atomic || (pos == 98304))) {
		pr_err("%s ATOMIC=%d pos=%lld len=%zd mapping_large_folio_support=%d\n", __func__,
			is_atomic, pos, len,
	...
	if (special_print && (is_atomic || (pos == 98304))) {
		pr_err("%s2 pos=%lld len=%zd calling __iomap_get_folio\n", __func__, pos, len);
	}
		...
		size_t copied, struct folio *folio)
{
	...
	if (srcmap->type == IOMAP_INLINE) {
		if (special_print)
			pr_err("%s2 folio=%pS (pos=%lld, size=%zd) copied=%zd pos=%lld len=%zd calling iomap_write_end_inline\n",
				__func__, folio, folio_pos(folio), folio_size(folio),
				copied, pos, len);
		ret = iomap_write_end_inline(iter, folio, pos, copied);
	} else if (srcmap->flags & IOMAP_F_BUFFER_HEAD) {
		if (special_print)
			pr_err("%s3 folio=%pS (pos=%lld, size=%zd) copied=%zd pos=%lld len=%zd calling block_write_end for IOMAP_F_BUFFER_HEAD\n",
				__func__, folio, folio_pos(folio), folio_size(folio),
				copied, pos, len);
		...
		if (special_print)
			pr_err("%s5 folio=%pS (pos=%lld, size=%zd) copied=%zd pos=%lld len=%zd calling __iomap_write_end\n",
				__func__, folio, folio_pos(folio), folio_size(folio),
				copied, pos, len);
...
static loff_t iomap_write_iter(struct iomap_iter *iter, struct iov_iter *i)
{
	...
	bool special_print = true;
	if (special_print && (is_atomic || (pos == 98304))) {
		pr_err("%s ATOMIC=%d pos=%lld len=%zd mapping=%pS iter->inode=%pS\n",
			__func__, is_atomic,
			pos, iov_iter_count(i), mapping, iter->inode);
	}
...
retry:
		...
		status = balance_dirty_pages_ratelimited_flags(mapping,
							       bdp_flags);
		if (special_print && (is_atomic || (pos == 98304))) {
			pr_err("%s1 retry: count=%d ATOMIC=%d pos=%lld len=%zd bytes=%zd offset=%zd\n",
				__func__, mycount, is_atomic, pos, iov_iter_count(i), bytes, offset);
		}
		mycount++;
		if (unlikely(status))
			break;
		...
		if (special_print && (is_atomic || (pos == 98304))) {
			...
			pr_err("%s2.1 iomap type=%d IOMAP_MAPPED=%d IOMAP_UNWRITTEN=%d set F_NEW=%d, DIRTY=%d, SHARED=%d, MERGED=%d, BUFFER_HEAD=%d, XATTR=%d\n",
				...
				!!(srcmap->flags & IOMAP_F_MERGED),
				!!(srcmap->flags & IOMAP_F_BUFFER_HEAD),
		...
		if (special_print && (is_atomic || (pos == 98304))) {
			//const struct iomap *srcmap = iomap_iter_srcmap(iter);
			//BUG_ON(!folio);
			pr_err("%s3 called iomap_write_begin pos=%lld len=%zd bytes=%zd offset=%zd folio=%pS (atomic=%d) folio_pos=%lld folio_size(folio)=%zd\n",
				__func__, pos, iov_iter_count(i), bytes, offset, folio, folio_test_atomic(folio),
		...
		if (special_print && (is_atomic || (pos == 98304))) {
			//const struct iomap *srcmap = iomap_iter_srcmap(iter);
			pr_err("%s5 calling copy_folio_from_iter_atomic pos=%lld len=%zd bytes=%zd offset=%zd copied=%zd folio=%pS\n",
				__func__, pos, iov_iter_count(i), bytes, offset, copied, folio);
		}
		...
		if (special_print && (is_atomic || (pos == 98304))) {
			//const struct iomap *srcmap = iomap_iter_srcmap(iter);
			pr_err("%s6 called iomap_write_end pos=%lld len=%zd bytes=%zd offset=%zd copied=%zd folio=%pS\n",
				__func__, pos, iov_iter_count(i), bytes, offset, copied, folio);
		}
...
new_ioend:
	...
	if (special_print)
		pr_err("%s2 folio=%pS (atomic=%d) pos=%lld len=%d calling bio_add_folio wpc->ioend->io_bio=%pS poff=%zd\n",
		__func__,
		...
		unsigned *count)
{
	...
	if (special_print)
		pr_err("%s folio=%pS (atomic=%d) pos=%lld dirty_len=%d\n", __func__,
			folio, folio_test_atomic(folio),
			pos, dirty_len);
	do {
		unsigned map_len;
...
#endif
	if (special_print)
		pr_err("%s folio=%pS (atomic=%d) pos=%lld end_pos=%lld iomap=%pS addr=%lld, offset=%lld, length=%lld wbc->range_start=%lld, range_end=%lld\n",
			__func__, folio, folio_test_atomic(folio),
			pos, end_pos, iomap, iomap->addr,
		...
		struct writeback_control *wbc, void *data)
{
	bool special_print = false;
	if (special_print)
		pr_err("%s folio=%pS (atomic=%d) folio_pos=%lld folio_size=%zd\n",
			__func__, folio, folio_test_atomic(folio),folio_pos(folio), folio_size(folio));
