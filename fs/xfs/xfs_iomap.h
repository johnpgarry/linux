...
xfs_aligned_fsb_count(
	...
	xfs_extlen_t		extsz)
{
	pr_err("%s offset_fsb=%lld count_fsb=%lld extsz=%d\n", __func__, offset_fsb, count_fsb, extsz);
	if (extsz) {
		xfs_extlen_t	align;
		...
		if (align)
			count_fsb += align;
		pr_err("%s1 after align to offset_fsb: count_fsb=%lld align=%d\n", __func__, count_fsb, extsz);
		div_u64_rem(count_fsb, extsz, &align);
		if (align)
			count_fsb += extsz - align;
		pr_err("%s2 after align to count_fsb: count_fsb=%lld align=%d\n", __func__, count_fsb, extsz);
	}

	pr_err("%s10 count_fsb=%lld\n", __func__, count_fsb);
	return count_fsb;
}
