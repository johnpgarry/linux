	...
	bool			force)
{
	...
	pr_err_once("%s3 end_fsb=%lld\n", __func__, end_fsb);
	if (XFS_IS_REALTIME_INODE(ip) && mp->m_sb.sb_rextsize > 1) {
		end_fsb = xfs_rtb_roundup_rtx(mp, end_fsb);
		pr_err_once("%s3.1 end_fsb=%lld XFS_IS_REALTIME_INODE(ip) && mp->m_sb.sb_rextsize > 1\n", __func__, end_fsb);
	}

	pr_err_once("%s xfs_inode_forcealign(ip)=%d\n", __func__, xfs_inode_forcealign(ip));
	if (xfs_inode_forcealign(ip)) {
		end_fsb = roundup_64(end_fsb, xfs_get_extsz_hint(ip));
		WARN_ON_ONCE(1);
		pr_err_once("%s3.3 end_fsb=%lld xfs_get_extsz_hint()=%d\n", __func__, end_fsb, xfs_get_extsz_hint(ip));
	}
