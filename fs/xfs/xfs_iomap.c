	...
	struct iomap		*srcmap)
{
	...
	//	pr_err("%s bodge to return -EIO for IOMAP_ATOMIC_COW\n", __func__);
	//	return -EIO;
	}
...
relock:
	...
	pr_err("%s0.3 after xfs_bmapi_read imap=%pS (startoff=%lld, startblock=%lld, blockcount=%lld, state=%d) agno=%d, agbno=%d filefrag physical offset=%d\n",
		__func__,
	...
	if (flags & IOMAP_ATOMIC_COW) {
		...
		error = xfs_reflink_allocate_cow(ip, &imap, &cmap, &shared,
				&lockmode, (flags & IOMAP_DIRECT) || IS_DAX(inode), true);
	...
	if (needs_cow) {
		error = -EAGAIN;
		pr_err("%s4 in needs_cow handling\n", __func__);
		if (flags & IOMAP_NOWAIT)
			goto out_unlock;
		...
		if (atomic) {
			bool found = false;
			pr_err("%s4.1 xfs_find_trim_cow_extent\n",
				__func__);
			error  = xfs_find_trim_cow_extent(ip, &imap, &cmap, &shared, &found);
			pr_err("%s4.1.0 called xfs_reflink_allocate_cow error=%d shared=%d found=%d\n",
...
//normal:
	...
	pr_err("%s4.9 calling xfs_bmbt_to_iomap length=%lld imap.startoff=%lld, startblock=%lld, blockcount=%lld, state=%d\n",
		__func__, length,
		imap.br_startoff, imap.br_startblock, imap.br_blockcount, imap.br_state);
