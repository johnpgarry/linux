...
retry:
	...
	/*
	 * REQ_ATOMIC-based atomic writes behave such that a racing read which
	 * overlaps with range being atomically written will see all or none of
	 * the old data.
	 * Emulate this behaviour for COW-based atomic writes by using
	 * IOMAP_DIO_FORCE_WAIT and inode_dio_wait() to ensure no active reads.
	 * This also locks out racing writes, which could trample on the COW
	 * fork extent.
	 */
