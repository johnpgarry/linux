...
struct xrep_cow {
	...
	struct xfo_bitmap	bad_fileoffs;
	...
	xfs_filblks_t		blockcount)
{
	...
	return xfo_bitmap_set(&xc->bad_fileoffs, startoff, blockcount);
}
	...
	struct xfs_scrub	*sc)
{
	...
	xc->sc = sc;
	xfo_bitmap_init(&xc->bad_fileoffs);
	...
	error = xfo_bitmap_walk(&xc->bad_fileoffs, xrep_cow_replace, xc);
	if (error)
		goto out_bitmap;
...
out_bitmap:
	...
	xfo_bitmap_destroy(&xc->bad_fileoffs);
	kmem_free(xc);
	return error;
}
