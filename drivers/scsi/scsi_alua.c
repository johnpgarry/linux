...
static char print_alua_state(unsigned char access_state)
{
	switch (access_state) {
...
void alua_print_info(struct scsi_device *sdev, int group_id, int access_state,
		int pref, int valid_states)
{
	sdev_printk(KERN_INFO, sdev,
		"alua: port group %02x state %c %s supports %c%c%c%c%c%c%c\n",
		group_id, print_alua_state(access_state),
...
blk_status_t scsi_alua_prep_cmd(struct scsi_cmnd *cmnd,
