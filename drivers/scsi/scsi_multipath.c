				...
				unsigned char access_state)
{
	...
	access_state &= SCSI_ACCESS_STATE_MASK;
	if (access_state == SCSI_ACCESS_STATE_OPTIMAL)
		state = MPATH_STATE_OPTIMIZED;
	else if (access_state == SCSI_ACCESS_STATE_ACTIVE)
		state = MPATH_STATE_OPTIMIZED;
	else
		state = MPATH_STATE_OTHER;

	WRITE_ONCE(sdev->scsi_mpath_dev->mpath_device.access_state, state);
}

/* Heavily based on alua_rtpg() in DH ALUA support */
static int scsi_mpath_alua_rtpg(struct scsi_device *sdev)
{
 ...
 retry:
	...
	if (ret) {
		 ...
		 * path failover.
		 */
		if ((scsi_mpath_dev->alua_valid_states &
			~TPGS_SUPPORT_OPTIMIZED) == 0) {
		...
		if (err == -EAGAIN &&
		    ...
		    time_before(jiffies, scsi_mpath_dev->alua_expiry)) {
	...
	if (orig_transition_tmo != scsi_mpath_dev->alua_transition_tmo) {
		...
		scsi_mpath_dev->alua_expiry = jiffies +
				scsi_mpath_dev->alua_transition_tmo * HZ;
	}
...
skip_rtpg:
	...
	case SCSI_ACCESS_STATE_TRANSITIONING:
		if (time_before(jiffies, scsi_mpath_dev->alua_expiry)) {
			...
			scsi_mpath_dev->alua_interval =
					SCSI_MPATH_ALUA_RTPG_RETRY_DELAY;
			err = -EAGAIN;
		} else {
			/* Transitioning time exceeded, set port to standby */
			err = -EIO;
			scsi_mpath_update_access_state(sdev,
...
static void scsi_mpath_alua_work(struct work_struct *work)
{
	...
	if (ret == -EAGAIN) {
		...
		sdev_printk(KERN_ERR, sdev, "error issuing RTPG (%d)\n", ret);
	}

	/* A path may have become enabled, so requeue any outstanding IO */
	mpath_schedule_requeue_work(mpath_head);
}
...
/* Check whether we send any commands */
static bool scsi_mpath_is_disabled(struct mpath_device *mpath_device)
{
	...
	if (access_state == SCSI_ACCESS_STATE_OFFLINE)
		return true;
...
static bool scsi_mpath_is_optimized(struct mpath_device *mpath_device)
{
	...
	/*
	 * Devices not supporting ALUA don't update access_state, but default
	 * is SCSI_ACCESS_STATE_OPTIMAL (0).
	 */
...
static bool scsi_mpath_available_path(struct mpath_device *mpath_device)
{
	...
	if (sdev_state == SDEV_CREATED || sdev_state == SDEV_CANCEL ||
	    sdev_state == SDEV_DEL || sdev_state == SDEV_OFFLINE ||
...
int __init scsi_multipath_init(void)
{
	...
	scsi_mpath_alua_wq = alloc_workqueue("scsi_mpath_alua_wq",
