				       ...
				       unsigned char flags, unsigned int dld)
{
	...
	if (write && 0)
		pr_err_once("%s scsi_sg_count=%d bio=%pS bio->bi_iter.bi_sector=%lld, bi_size=%d write=%d\n",
			__func__, scsi_sg_count(cmd),
			bio,
			bio ? bio->bi_iter.bi_sector : 0,
				       ...
				       unsigned char flags, unsigned int dld)
{
	...
	put_unaligned_be32(nr_blocks, &cmd->cmnd[10]);
	if (write && 0)
		pr_err("%s scsi_sg_count=%d bio=%pS bio->bi_iter.bi_sector=%lld, bi_size=%d write=%d\n",
			__func__, scsi_sg_count(cmd),
			bio,
			bio ? bio->bi_iter.bi_sector : 0,
			bio ? bio->bi_iter.bi_size : 0,
				      ...
				      unsigned char flags)
{
	...
	if (write && 0)
		pr_err("%s scsi_sg_count=%d write=%d\n", __func__, scsi_sg_count(cmd), write);
					...
					bool boundary, unsigned char flags)
{
	...
	pr_err("%s scsi_sg_count=%d bio=%pS bio->bi_iter.bi_sector=%lld, bi_size=%d nr_blocks=%d\n",
			__func__, scsi_sg_count(cmd),
			bio,
			bio ? bio->bi_iter.bi_sector : 0,
			bio ? bio->bi_iter.bi_size : 0,
...
static blk_status_t sd_setup_read_write_cmnd(struct scsi_cmnd *cmd)
{
	...
	if (write && (rq->cmd_flags & REQ_ATOMIC))
		pr_err("%s scsi_sg_count=%d bio=%pS bi_next=%pS bio->bi_iter.bi_sector=%lld, bi_size=%d write=%d REQ_ATOMIC=%d rq->__data_len=%d\n",
			__func__, scsi_sg_count(cmd),
			bio, bio->bi_next,
			bio ? bio->bi_iter.bi_sector : 0,
			bio ? bio->bi_iter.bi_size : 0,
			...
			rq->__data_len);
	//WARN_ON((write == 0) && (bio->bi_iter.bi_sector == 0) && (bio->bi_iter.bi_size == 4096));
