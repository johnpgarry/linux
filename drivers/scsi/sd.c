...
static void sd_config_atomic(struct scsi_disk *sdkp)
{
	...
	if (!sdkp->max_atomic || sdkp->protection_type == T10_PI_TYPE2_PROTECTION)
		return;

	physical_block_size_sectors = sdkp->physical_block_size /
					sdkp->device->sector_size;
	max_atomic = max_t(unsigned int, rounddown_pow_of_two(sdkp->max_atomic),
	...
	blk_queue_virt_boundary(q, 0xfff);
}
