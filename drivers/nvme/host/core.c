		...
		struct nvme_ns *ns, struct nvme_id_ns *id)
{
	...
	if (id->nsfeat & NVME_NS_FEAT_ATOMICS && id->nawupf) {
		if (id->nabo) {
			dev_err(ns->ctrl->device, "Support atomic writes NABO=%d\n",
				id->nabo);
		} else {
			u32 boundary;
			 ...
			 * of atomic_bs (and not necessarily a power-of-2), so
			...
			if (!boundary || is_power_of_2(boundary)) {
				...
				dev_err(ns->ctrl->device, "Unsupported atomic write boundary (%d)\n",
					boundary);
			}
		}
