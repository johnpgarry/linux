		...
		struct queue_limits *lim)
{
	...
	if (id->nsfeat & NVME_NS_FEAT_IO_OPT) {
		...
		phys_bs = bs * (1 + 1 /*le16_to_cpu(id->npwg)*/);
		pr_err("%s2 NVME_NS_FEAT_IO_OPT set=%d ns=%pS id=%pS phys_bs=%d\n",
			__func__, !!(id->nsfeat & NVME_NS_FEAT_IO_OPT), ns, id, phys_bs);
...
static int nvme_init_subsystem(struct nvme_ctrl *ctrl, struct nvme_id_ctrl *id)
{
	...
	subsys->cmic = id->cmic = NVME_CTRL_CMIC_MULTI_CTRL;
