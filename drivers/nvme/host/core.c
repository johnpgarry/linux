...
static void nvme_update_atomic_write_disk_info(struct gendisk *disk,
		struct nvme_ns *ns, struct nvme_id_ns *id, u32 atomic_bs)
{
	...
	if (id->nsfeat & NVME_NS_FEAT_ATOMICS && id->nawupf) {
		if (id->nabo) {
			dev_notice(ns->ctrl->device, "Atomic writes not supported for NABO set (%d)\n",
				id->nabo);
		} else {
			...
			if (!boundary || is_power_of_2(boundary)) {
				...
				dev_notice(ns->ctrl->device, "Unsupported atomic write boundary (%d)\n",
					boundary);
				boundary = 0;
			}
		}
		...
		struct nvme_ns *ns, struct nvme_id_ns *id)
{
	...
	if (id->nabo == 0) {
		...
		nvme_update_atomic_write_disk_info(disk, ns, id, atomic_bs);
	}
	...
	blk_queue_max_write_zeroes_sectors(q, ns->ctrl->max_zeroes_sectors);
}
