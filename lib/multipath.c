...
{
	pr_err("%s calling cleanup_srcu_struct\n", __func__);
...
void mpath_head_uninit(struct mpath_head *mpath_head)
{
	...
	pr_err("%s mpath_head=%pS count before=%d\n",
		__func__, mpath_head, refcount_read(refcount));
	// wait for all references to get cleared

	if (refcount_dec_and_test(refcount)) {
		mpath_head_cleanup(mpath_head);
	} else {
		pr_err("%s1 going to wait\n", __func__);
