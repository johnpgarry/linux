...
int main(int argc, char **argv)
{
	...
	printf("file=%s write_size=%d pos=%d o_flags=0x%x wr_flags=0x%x multi_vector_alloc_size=%d\n",
	...
	for (vectors = 0, i = 0; i < max_vectors && remain > 0; i++, vectors++) {
		...
		if (use_iovec) {
			...
			my_buf += offset;
			printf("4 i=%d my_buf=%p remain=%d offset=%d sz=%d &_cbs[i]=%p pos_tmp=%lld\n", i, my_buf, remain, offset, sz, &_cbs[i], pos_tmp);
...
do_write:
	...
	for (i = 0; i < vectors; i++) {
		...
		if (_cbs[i].u.c.nbytes)
			printf("cbs[%d].u.c.buf=0x%x, .nbytes=%d offset=%d\n", 
				i, _cbs[i].u.c.buf, _cbs[i].u.c.nbytes, _cbs[i].u.c.offset);
		if (iov[i].iov_len)
			printf("iov[%d].iov_base=0x%x, .iov_len=%d offset=%d %s\n", 
