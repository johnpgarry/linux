...
static void print_extent_info(struct fiemap_extent *fm_extent, int cur_ex,
			      ...
			      ext2fs_struct_stat *st, int alignment)
{
...
#if 0
	printf("%s1 xyz logical_width=%d\n", __func__, logical_width);
	printf("%s2 xyz logical_blk=%d\n", __func__, logical_blk);
	printf("%s3 xyz logical_width=%d\n", __func__, logical_width);
	printf("%s4 xyz logical_blk=%d\n", __func__, logical_blk);
	printf("%s5 xyz logical_width=%d\n", __func__, logical_width);
	printf("%s6 xyz logical_blk=%d\n", __func__, logical_blk);
	printf("%s7 xyz physical_width=%d\n", __func__, physical_width);
	printf("%s8 xyz physical_blk=%d\n", __func__, physical_blk);
	printf("%s9 xyz ext_blks_phys=%d\n", __func__, ext_blks_phys);
	printf("%s10 xyz ext_blks=%d\n", __func__, ext_blks);
	printf("%s11 xyz ext_len=%d\n", __func__, ext_len);
	printf("%s99 xyz physical_width=%d alignment=%d\n", __func__, physical_width, alignment);
	#endif
	...
	if (alignment && physical_blk % alignment) {
		...
		} else if (fm_extent->fe_flags & FIEMAP_EXTENT_SHARED) {
			printf("%s warning physical_blk=%lld alignment=%d SHARED\n", __func__, physical_blk, alignment);
		} else {
...
static int filefrag_fibmap(int fd, int blk_shift, int *num_extents,
			   ...
			   unsigned long numblocks, int is_ext2, int alignment)
{
		...
		if (*num_extents == 0 || block != last_block + 1 ||
			...
			if (force_extent && *num_extents != 0) {
				print_extent_info(&fm_ext, *num_extents - 1,
						  expected, blk_shift, st, alignment);
			}
...
int frag_report(const char *filename, int alignment)
{
	...
	if (!force_bmap) {
		rc = filefrag_fiemap(fd, blk_shift, &num_extents, &st, alignment);
		expected = 0;
		if (rc < 0 &&
	...
	if (force_bmap || rc < 0) { /* FIEMAP failed, try FIBMAP instead */
		expected = filefrag_fibmap(fd, blk_shift, &num_extents,
					   &st, numblocks, is_ext2, alignment);
		if (expected < 0) {
